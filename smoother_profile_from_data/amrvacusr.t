...
{do ix^D=ixOmin^D, ixOmax^D \}
    ! Sample the last 1.0 unit of the data slab to get the reference state
    if (x(ix^D,3) >= 21.0d0 .and. x(ix^D,3) < 22.0d0) then
...
if (count_global > 0) then
    ...
    ! Dynamically calculate kappa from the EOS: P = kappa * rho^gamma
    kappa = pp_avg_ri / (rho_avg_ri**eqpar(gamma_))
else
    ! Fallback defaults in case the sampling region is empty
