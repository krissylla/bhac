...
!    call mpistop('stopping after reading')
  ...
  subroutine initonegrid_usr(ixI^L,ixO^L,s)
    ...
    double precision                         :: rho_sum_local, rho_sum_global, rho_avg_ri
    double precision                         :: pp_sum_local,  pp_sum_global,  pp_avg_ri
...
! --- Step 1: Calculate Volume Average Density and Pressure (z < 22.0) ---
rho_sum_local = 0.0d0
pp_sum_local  = 0.0d0
count_local   = 0
r_i = 22.0d0 + 117.6d0 
...
{do ix^D=ixOmin^D, ixOmax^D \}
    ! Now sampling EVERYTHING below the interface
    if (x(ix^D,3) < 22.0d0) then
        call interpolate_oneblock(x(ix^D,:), rho_, w(ix^D, rho_))
        call interpolate_oneblock(x(ix^D,:), pp_,  w(ix^D, pp_))
        
        rho_sum_local = rho_sum_local + w(ix^D, rho_)
        pp_sum_local  = pp_sum_local  + w(ix^D, pp_)
        count_local   = count_local   + 1
    end if
{enddo\}

! Global MPI Reduction
call MPI_ALLREDUCE(rho_sum_local, rho_sum_global, 1, MPI_DOUBLE_PRECISION, MPI_SUM, icomm, ierror)
call MPI_ALLREDUCE(pp_sum_local,  pp_sum_global,  1, MPI_DOUBLE_PRECISION, MPI_SUM, icomm, ierror)
call MPI_ALLREDUCE(count_local,   count_global,   1, MPI_INTEGER,          MPI_SUM, icomm, ierror)

if (count_global > 0) then
    rho_avg_ri = rho_sum_global / dble(count_global)
    pp_avg_ri  = pp_sum_global  / dble(count_global)
    kappa      = pp_avg_ri / (rho_avg_ri**eqpar(gamma_))
else
    ! Fallback
    rho_avg_ri = 2.325789d-11
    kappa      = 22853.548855129946d0
end if

! --- Step 2: Main Grid Initialization Loop ---
{do ix^D=ixOmin^D, ixOmax^D \}
    if (x(ix^D,3) < 22.0d0) then
        ! Note: The interpolation for the lower region was already done in the 
        ! sampling loop above, so we don't need to call interpolate_oneblock again!
        ! We just need to handle velocities and potential other variables.
        ...
        ! Outer Region: Spherical Atmosphere
