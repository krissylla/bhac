...
!  INCLUDE:amrvacnul/usrflags.t
  ...
  subroutine initglobaldata_usr
    ...
    call read_oneblock("test_it_53760.blk")
...
{do ix^D=ixOmin^D, ixOmax^D \}
    ...
    w(ixO^S, rho_) = (x(ix^D,1)**2+x(ix^D,2)**2+(x(ix^D,3)+117.6)**2)**-1 * 117.6**2 * 2.325789d-11
    !w(ixO^S, rho_) = 1.0d-12
    w(ixO^S, pp_) = 22853.548855129946 * w(ixO^S, rho_)**(eqpar(gamma_)) 
    w(ixO^S, u1_) = 0.0d0
    w(ixO^S, u2_) = 0.0d0
    w(ixO^S, u3_) = 0.0d0
	end if
